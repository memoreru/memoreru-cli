...
| `icon` | object\|null | アイコン（絵文字 or 画像）: `{"type":"emoji","emoji":"📝"}` / `{"type":"image","path":"./icon.png"}` / `null` |
...
- 列名 / `required` / `description` の変更
- 型変更（**同一ストレージのみ**。例 `string→select`, `number→rating`）。ストレージ非互換な型変更（例 `string→number`）は安全のためスキップされます（列削除→再作成で対応）。

#### 列の削除（`--delete-columns`）

列を削除する場合は **column_id を明示指定**します。CSV から列を外しただけでは削除されません（誤削除防止）。

```bash
memoreru push --preview --delete-columns <id1>,<id2>   # 削除予定の確認（削除しない）
memoreru push --delete-columns <id1>,<id2>             # 削除して更新
```

- 削除は**明示指定した column_id**（当該 content に属するもの）のみが対象です。
- 実際に削除できるかは**サーバ側のポリシー・権限**に従います（権限が無い場合は無視されます）。
- 通常の push（`--delete-columns` なし）では列を削除しません。
